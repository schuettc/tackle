...
func printSync(out io.Writer, machine string, r app.SyncReport) {
	...
	if r.Offline {
		fmt.Fprintln(out, "offline: the docket remote is unreachable; changes are queued locally")
	}
	for _, f := range r.Resolved {
		fmt.Fprintf(out, "resolved a decision race in %s (later decision kept; see `docket attention`)\n", f)
	}
	...
	fmt.Fprintf(out, "attention: %d item(s) (docket attention)\n", r.Attention)
}
