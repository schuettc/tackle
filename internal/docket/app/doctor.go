...
// Doctor inspects this machine's docket setup. exe is the running binary,
...
func (a *App) Doctor(ctx context.Context, h hooks.Options, exe string) []Check {
	...
	switch {
	case err != nil:
		add("docket repo", "fail", "%s: %v", a.Repo.Dir, err)
	case st != "":
		add("docket repo", "warn", "%s has uncommitted changes (a sync will commit them)", a.Repo.Dir)
	default:
		queued, _ := gitx.Run(ctx, a.Repo.Dir, "rev-list", "--count", "origin/main..HEAD")
		if queued != "" && queued != "0" {
			add("docket repo", "warn", "%s commits not pushed yet", queued)
		} else {
			add("docket repo", "ok", "%s", a.Repo.Dir)
		}
	}
	if _, err := gitx.Run(ctx, a.Repo.Dir, "ls-remote", "-q", "origin", "HEAD"); err != nil {
		add("remote", "warn", "%s unreachable: %v", a.Cfg.DocketRemote, err)
	} else {
		add("remote", "ok", "%s", a.Cfg.DocketRemote)
	}
	...
	switch {
	case !hs.Installed:
		add("hooks", "warn", "not installed; git actions are not journaled (run `docket hooks install`)")
	case !hs.BinaryOK:
		add("hooks", "fail", "shims call %s, which is missing (re-run `docket hooks install`)", hs.Binary)
	case len(hs.Missing) > 0:
		add("hooks", "warn", "shims missing for %s (re-run `docket hooks install`)", strings.Join(hs.Missing, ", "))
	...
	if err != nil {
		...
		if len(g.Owners) == 0 {
			add("github cache", "warn", "empty (run `docket sync`)")
		}
	}
