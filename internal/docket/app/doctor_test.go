...
func TestDoctor(t *testing.T) {
	...
	h := hooks.Options{Dir: filepath.Join(t.TempDir(), "hooks"), StatePath: filepath.Join(t.TempDir(), "hooks.json"), Binary: "/nonexistent/docket"}
	checks := r.app.Doctor(ctx, h, "/nonexistent/docket")
	byName := map[string]Check{}
	for _, c := range checks {
		byName[c.Name] = c
	}
	for name, want := range map[string]string{"config": "ok", "docket repo": "ok", "remote": "ok", "gh": "ok", "hooks": "warn", "owner Acme": "warn"} {
