...
type rig struct {
	opts Options
	log  string // what the stub "docket" received
	repo string
}
...
func setup(t *testing.T) rig {
	...
	stub := filepath.Join(base, "docket-stub")
	script := "#!/bin/sh\n{ echo \"ARGS $*\"; cat; } >>'" + log + "'\n"
...
func TestShimPrePushStdinReachesBoth(t *testing.T) {
	...
	if !strings.Contains(log, "ARGS hook pre-push origin "+remote) || !strings.Contains(log, "refs/heads/main "+head) {
		t.Errorf("docket pre-push record:\n%s", log)
	}
}
...
func TestDocketDisableSkipsRecordingOnly(t *testing.T) {
	r := setup(t)
	r.install(t)
	marker := filepath.Join(t.TempDir(), "ran")
	repoHook(t, r.repo, "post-commit", "touch '"+marker+"'")
	t.Setenv("DOCKET_DISABLE", "1")
	testgit.Commit(t, r.repo, "h", "z")
	if r.stubLog(t) != "" {
		t.Errorf("recorded despite DOCKET_DISABLE:\n%s", r.stubLog(t))
	}
	if _, err := os.Stat(marker); err != nil {
		t.Error("repo hook skipped under DOCKET_DISABLE")
	}
}
...
func TestShimsAreValidPOSIX(t *testing.T) {
	...
	for _, name := range Names {
		src := Shim(name, "/opt/it's/docket", "/prev dir")
