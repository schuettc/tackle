# docket repository format, version 1

A docket data repository is a private git repository written only by the
`docket` binary. History is linear: no merge commits, no branches.

## Root

| Path | Content |
|---|---|
| `docket.toml` | `format_version = 1`. A docket binary refuses a repo with a newer version. |
...
The GitHub observation cache (`~/.local/state/docket/github.json`), the drift record (`seen.json`), the event spool and the hook shims are machine-local.
