...
func home(t *testing.T) string {
	...
	h := t.TempDir()
	t.Setenv("HOME", h)
	t.Setenv("DOCKET_HOME", filepath.Join(h, "docket-home"))
	return h
}
...
func TestSaveLoadRoundTripAndDefaults(t *testing.T) {
	...
	if err := Save(Config{User: "schuettc", DocketRemote: "git@github.com:schuettc/docket-data.git", Roots: []string{"~/code"}}); err != nil {
		t.Fatal(err)
	}
	fi, err := os.Stat(Path())
	...
	if !strings.HasPrefix(c.DocketRepo, filepath.Join(h, "docket-home")) {
		t.Errorf("docket repo %q not under DOCKET_HOME", c.DocketRepo)
	}
	var d Config
	d.Defaults()
...
func TestDirsLiveUnderToolHome(t *testing.T) {
	h := home(t)
	base := filepath.Join(h, "docket-home")
