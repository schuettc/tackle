// Package config is the per-machine docket configuration, stored at
// tools.ConfigDir("docket")/config.toml. Settings shared by every machine
// (policy) live in the docket repo instead.
package config
...
// Tool is the family tool name used for the docket's directories.
const Tool = "docket"
...
type Config struct {
	...
	DocketRepo   string   `toml:"docket_repo"`
	DocketRemote string   `toml:"docket_remote"`
	Roots        []string `toml:"roots"`
	Owners       []string `toml:"owners,omitempty"`
	SyncInterval string   `toml:"sync_interval"`
}

// ErrNotInitialized means this machine has no docket config yet.
var ErrNotInitialized = errors.New("docket is not initialized on this machine")

// Path is the config file.
func Path() string { return filepath.Join(tools.ConfigDir(Tool), "config.toml") }

// StateDir is the docket's machine-local state directory.
...
// HooksDir is where `docket hooks install` writes the global hook shims.
...
// Defaults fills unset fields (machine = short host name, docket repo under
...
func (c *Config) Defaults() {
	...
	if c.DocketRepo == "" {
		c.DocketRepo = filepath.Join(tools.DataDir(Tool), "repo")
	}
	if len(c.Roots) == 0 {
	...
	c.DocketRepo = Expand(c.DocketRepo)
	for i := range c.Roots {
