...
func newStore(t *testing.T) (*Repo, string) {
	...
	r, err := Init(ctx, filepath.Join(t.TempDir(), "docket"), remote)
	if err != nil {
		t.Fatal(err)
	}
	return r, remote
...
func TestInitBootstrapsEmptyRemote(t *testing.T) {
	r, remote := newStore(t)
	for _, f := range []string{"docket.toml", "policy.toml", "README.md"} {
		if _, err := os.Stat(filepath.Join(r.Dir, f)); err != nil {
			t.Errorf("%s missing: %v", f, err)
		}
	}
	if got := testgit.Git(t, remote, "log", "--format=%s", "main"); !strings.Contains(got, "init docket") {
		t.Errorf("remote log %q", got)
	}
	// A second machine's init clones the existing docket.
	r2, err := Init(ctx, filepath.Join(t.TempDir(), "docket2"), remote)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenRejectsNewerFormat(t *testing.T) {
	r, _ := newStore(t)
	if err := os.WriteFile(filepath.Join(r.Dir, "docket.toml"), []byte("format_version = 99\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestValidateReportsBadFiles(t *testing.T) {
	...
	if errs := r.Validate(); len(errs) != 0 {
		t.Fatalf("fresh docket invalid: %v", errs)
	}
