// Package testgit gives docket tests hermetic git: no user or system config,
// a fixed identity, and no inherited harness or docket variables.
package testgit
...
// Env isolates git and the docket's environment for the rest of the test.
func Env(t testing.TB) {
	...
	for _, k := range []string{"CLAUDE_CODE_SESSION_ID", "AGENT_SESSION_ID", "AGENT_SESSION_CHILD",
		"DOCKET_INTERNAL", "DOCKET_DISABLE", "GIT_DIR", "GIT_WORK_TREE", "GIT_INDEX_FILE"} {
		t.Setenv(k, "")
		os.Unsetenv(k)
	}
}
