...
import (
	...
	"github.com/schuettc/tackle/internal/ledger/record"
	...
	return []tools.Command{
		{
			...
			Run: func(args []string, out, errw io.Writer) error {
				...
				fmt.Fprintf(out, "ledger initialized: machine %s, user %s, repo %s\nnext: ledger hooks install && ledger sync\n", cfg.Machine, cfg.User, cfg.LedgerRepo)
				return nil
			},
		},
		{
		...
		},
		{
			...
			Run: func(args []string, out, errw io.Writer) error {
				...
				if !ok {
					return tools.Exitf(1, "%s is not a known item", k).WithHint("ledger attention")
				}
				evs, _ := a.Events()
		...
		},
		{
			...
			Run: func(args []string, out, errw io.Writer) error {
				...
				if err != nil {
					return tools.Exitf(1, "%v", err).WithHint("ledger decide --help")
				}
		...
		},
		{
			...
			Run: func(args []string, out, errw io.Writer) error {
				...
				fmt.Fprintf(out, "wrote %d item(s) to %s; fill it in, then: ledger decide --from %s\n", len(items), dest, dest)
				return nil
			},
		},
		{
...
func initFlags() *flag.FlagSet {
	return flags("init", "ledger init <remote> [flags]", "Idempotent. The first machine bootstraps an empty remote.", func(fs *flag.FlagSet) {
...
func syncFlags() *flag.FlagSet {
	return flags("sync", "ledger sync [flags]", "Runs from launchd every 30 minutes once layer 1b is installed; safe to run any time.", func(fs *flag.FlagSet) {
...
func filterFlags(name string) func() *flag.FlagSet {
	return flags(name, "ledger "+name+" [flags]", "", func(fs *flag.FlagSet) {
...
func decideFlags() *flag.FlagSet {
	return flags("decide", "ledger decide <key> <disposition> [flags] | ledger decide --from <file>", "", func(fs *flag.FlagSet) {
...
func triageFlags() *flag.FlagSet {
	return flags("triage", "ledger triage [flags]", "", func(fs *flag.FlagSet) {
		fs.String("kind", "", "only this kind")
		fs.String("repo", "", "only items of this owner/name")
		fs.String("out", "ledger-triage.toml", "worksheet path ('-' = stdout)")
	})()
}

func briefFlags() *flag.FlagSet {
	return flags("brief", "ledger brief [--cwd DIR] [--max N]", "", func(fs *flag.FlagSet) {
...
func recordFlags() *flag.FlagSet {
	return flags("record", "ledger record --harness claude|pi < payload.json", "", func(fs *flag.FlagSet) {
		fs.String("harness", "claude", "payload format: claude (PostToolUse) or pi (pi-ledger)")
	})()
}
