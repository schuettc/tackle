// Package cli is the ledger command line, built on the tools-common App.
package cli
...
// Main runs `ledger args...` and returns the exit code. `ledger hook` is
...
func open() (*app.App, error) {
	...
	if errors.Is(err, config.ErrNotInitialized) {
		return nil, tools.Exitf(1, "ledger is not initialized on this machine").WithHint("ledger init git@github.com:<you>/ledger-data.git")
	}
	return a, err
}
