...
// ExecRunner runs the real gh CLI (auth is gh's own; the ledger never sees a token).
type ExecRunner struct{}

// Gh runs gh with args.
func (ExecRunner) Gh(ctx context.Context, args ...string) ([]byte, error) {
	cmd := exec.CommandContext(ctx, "gh", args...)
	cmd.Env = append(os.Environ(), "LEDGER_INTERNAL=1", "GH_PROMPT_DISABLED=1", "NO_COLOR=1", "GH_NO_UPDATE_NOTIFIER=1")
