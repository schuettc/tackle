// Package store is the ledger data repository: a git repo that holds decisions
// (items/), journals (journal/), machine snapshots (machines/), views and
// shared policy. The ledger binary is its only writer; history is linear.
package store
...
// FormatVersion is the ledger repo format this binary reads and writes
// (ledger.toml format_version). Documented in internal/ledger/FORMAT.md.
const FormatVersion = 1

const readme = "# ledger\n\nThis repository is written by `ledger sync`. Views appear here after the first sync.\n"

// Repo is an open ledger data repository.
type Repo struct {
	Dir string
}
...
// Init prepares dir as a ledger clone of remote. An existing clone at dir is
// opened. An empty remote is bootstrapped (ledger.toml, policy.toml, README.md)
// and pushed.
func Init(ctx context.Context, dir, remote string) (*Repo, error) {
	...
	if _, err := gitx.Run(ctx, filepath.Dir(dir), "clone", "-q", remote, dir); err != nil {
		return nil, fmt.Errorf("clone ledger repo: %w", err)
	}
	...
	for rel, b := range map[string][]byte{
		"ledger.toml": fmt.Appendf(nil, "# ledger data repository. Format: internal/ledger/FORMAT.md in schuettc/tackle.\nformat_version = %d\n", FormatVersion),
		"policy.toml": pol,
	...
	if _, err := r.Commit(ctx, "init ledger"); err != nil {
		return nil, err
	}
	if _, err := gitx.Run(ctx, dir, "push", "-q", "-u", "origin", "main"); err != nil {
		return nil, fmt.Errorf("push new ledger: %w", err)
	}
	return r, nil
}

// Open opens an existing ledger clone and checks its format version.
func Open(dir string) (*Repo, error) {
	b, err := os.ReadFile(filepath.Join(dir, "ledger.toml"))
	if err != nil {
		return nil, fmt.Errorf("not a ledger repo (%s): %w", dir, err)
	}
	var m meta
	if _, err := toml.Decode(string(b), &m); err != nil {
		return nil, fmt.Errorf("ledger.toml: %w", err)
	}
	if m.FormatVersion < 1 || m.FormatVersion > FormatVersion {
		return nil, fmt.Errorf("ledger.toml: format_version %d is not supported by this ledger (max %d); update ledger", m.FormatVersion, FormatVersion)
	}
