...
// Main is `ledger hook <name> [args...]`, run by the shims. It never fails,
// prints nothing, runs no git and no network, and returns within a second.
// On a machine without a ledger config it records nothing.
func Main(args []string, stdin io.Reader) int {
	if len(args) == 0 || os.Getenv("LEDGER_DISABLE") != "" || os.Getenv("LEDGER_INTERNAL") != "" {
		return 0
	}
