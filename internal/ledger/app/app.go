// Package app runs the ledger's commands against the real stores: it wires
// config, the ledger repo, the spool, observation and the engine together.
package app
...
// App is an initialized ledger on this machine.
type App struct {
	Cfg  config.Config
	Repo *store.Repo
	Gh   observe.Runner
...
// Open loads this machine's config and ledger repo.
func Open(gh observe.Runner) (*App, error) {
	cfg, err := config.Load()
	if err != nil {
		return nil, err
	}
	repo, err := store.Open(cfg.LedgerRepo)
	if err != nil {
		return nil, fmt.Errorf("%w (run `ledger init %s` to re-clone it)", err, cfg.LedgerRemote)
	}
...
// clones or bootstraps the ledger repo. Safe to re-run.
func Init(ctx context.Context, o InitOptions, gh observe.Runner) (config.Config, error) {
	cfg, err := config.Load()
	switch {
	case err == nil:
		if o.Remote != "" && cfg.LedgerRemote != o.Remote {
			return cfg, fmt.Errorf("already initialized with remote %s (edit %s to change it)", cfg.LedgerRemote, config.Path())
		}
	case errors.Is(err, config.ErrNotInitialized):
		if o.Remote == "" {
			return cfg, fmt.Errorf("init needs the ledger repo remote, e.g. git@github.com:<you>/ledger-data.git")
		}
		user := o.User
		if user == "" {
		...
		cfg = config.Config{Machine: o.Machine, User: user, LedgerRemote: o.Remote, Roots: o.Roots}
		cfg.Defaults()
	...
	if _, err := store.Init(ctx, cfg.LedgerRepo, cfg.LedgerRemote); err != nil {
		return cfg, err
	}
	return cfg, nil
}
...
func (a *App) input(g *observe.GitHub, snaps []observe.Snapshot) (engine.Input, error) {
	...
	if len(errs) > 0 {
		return engine.Input{}, fmt.Errorf("unreadable decision files (run `ledger validate`): %w", errors.Join(errs...))
	}
...
// snapshots and decisions in the ledger repo. No network, no writes.
...
// Events reads every journal line in the ledger repo, oldest file first.
